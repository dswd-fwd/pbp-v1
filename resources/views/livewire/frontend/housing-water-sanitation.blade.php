...
class extends Component {
    ...
    public function mount()
    {
        ...
        $this->user = Auth::user();
        $this->section = Section::with('questions.options.subOptions')->where('name', 'HOUSING, WATER AND SANITATION')->first();
    ...
    public function submitAnswer()
    {
        ...
        foreach ($this->final_answers as $question_id => $answerData) {
            ...
            // **Step 2: Remove previous AnswerOptions if user is changing their answer**
            ...
            if (isset($answerData['option_id'])) {
                ...
                $otherText = $answerData['option_has_other_text'] ?? null;
                ...
                // **Step 4: Remove previous AnswerSubOptions**
                $answerOption->answerSubOptions()->delete();

                // Step 5: Handle sub-options
                if (!empty($answerData['option_child_id'])) {
                    $subOptionId = $answerData['option_child_id'];
                    $childOtherText = $answerData['option_child_has_other_text'] ?? null;

                    AnswerSubOption::create([
                        'answer_option_id' => $answerOption->id,
                        'sub_option_id' => $subOptionId,
                        'other_text' => $childOtherText,
                    ]);
                }
            }

            // Step 6: Handle multiple answers (Checkbox Selections)
            if (!empty($this->final_answers_checkbox[$question_id])) {
                foreach ($this->final_answers_checkbox[$question_id] as $option_id => $checkboxData) {
                    AnswerOption::create([
                        'answer_id' => $answer->id,
                        'option_id' => $option_id,
        ...
        $this->redirect(route('health-and-nutrition'), navigate: true);
    }
...
<div class="w-full max-w-4xl p-10 m-auto overflow-hidden bg-white border border-zinc-200 text-zinc-800 rounded-xl">
    ...
    {{-- <pre>Answers: {{ print_r($answers, true) }}</pre>
    <pre>Other Answers: {{ print_r($other_answers, true) }}</pre>
    <pre>Answers with child: {{ print_r($answers_with_child, true) }}</pre>
    <pre>Multiple Answers: {{ print_r($multiple_answers, true) }}</pre>
    <pre>Answers with child: {{ print_r($answers_with_child, true) }}</pre>
    <pre>Final Answers: {{ print_r($final_answers, true) }}</pre> --}}
    {{-- @dump($existing_answers) --}}
    <form wire:submit="submitAnswer">
                                                                ...
                                                                @foreach($option->subOptions as $sub_option)
                                                                    <label class="flex items-center ml-8 space-x-2" wire:key="sub_option-{{ $index }}">
                                                                        <input 
                                                                            ...
                                                                            wire:change="updateSelection({{ $question->id }}, {{$sub_option['has_other']}}, {{ $option['id'] }}, null, null, null, null, null, {{ $sub_option->id }}, null)"
                                                                            {{ !in_array($sub_option->option_id, $answers) ? 'disabled' : '' }}
                                                                        >
                                                                        <span>{{ $sub_option['sub_option_text'] }}{{ $sub_option->id }}</span>
                                                                    </label>
                                                                    @if ($sub_option['has_other'])
        ...
        <div class="flex justify-end gap-4 mt-8">
            <div class="w-full">
                <a wire:navigate href="{{ route('family-profile') }}">
    ...
    <div class="flex justify-center mt-16">
        <x-form.progress-indicator :start="1" :end="10" :current="3" />
    </div>
</div>
